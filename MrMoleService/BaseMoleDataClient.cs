...
public abstract class BaseMoleDataClient
{
    #region Base
    ...
    /// <returns></returns>
    protected async Task<TEntity> GetAsync<TEntity>(string url, CancellationToken Cancel = default) where TEntity : IMoleResponse, new()
    {
        Remaining -= 1;
    ...
    /// <param name="Cancel">Признак отмены асинхронной операции</param>
    /// <returns></returns>
    protected async Task<TEntity> PostAsync<TItem, TEntity>(string url, TItem item, CancellationToken Cancel = default) where TEntity : IMoleResponse, new()
    {
        Remaining -= 1;
    ...
    protected BaseMoleDataClient(bool waitWhenLimit, string apiKey, string BaseServiceAddress = "https://mrmole.org/api")
    {
